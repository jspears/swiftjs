...
export class NSDictionaryClass implements Record<string, unknown> {
    ...
    constructor(...[objects,keys]:([URL, ErrorFn?] |[AnyHashable, boolean?] | [unknown, string] | [unknown[], string[] | []])){
    ...
    keysSortedByValue = (comparator:Selector)=>{
        return Array.from(this.dict.entries()).sort(([,va], [,vb])=>comparator(va,vb)).map(v=>v[0]);
    }
