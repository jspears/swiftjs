...
describe('ParamParse', function(){
    ...
    it.only('should parse "func insert(AccessibilityTraits) -> (inserted: Bool, memberAfterInsert: AccessibilityTraits)"', function(){
        ...
        const ret = ClosureImpl.parseMethod("func insert(AccessibilityTraits) -> (inserted: Bool, memberAfterInsert: AccessibilityTraits)", addType)
...
const oneArg= <T extends AnyFn> (...expected:ReturnType<T>[]):[T, ()=>void]=>{
    ...
    }) as T, ()=>expect(result).to.eql(expected)]
}
