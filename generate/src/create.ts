...
const closureParse = (str: string) => {
  ...
  const parameters = split(argType)
    .map(fixNameType)
    .filter(Boolean) as NameType[];
  return {
    parameters,
    returnType,
  };
...
export function fragToMethod(
  f: ReferenceType['fragments'] = [],
  ignore = new Set()
) {
  const str = Array.isArray(f) ? f.map((v) => v.text).join('') : f;

  const reg =
    /func\s+?(.+?)(?:<(.+?)>)?\(\s*(.*)\s*\)\s*->\s*(?:some\s*)?(.+?)(?:<(.+?)>)?$/.exec(
      str
    ) || [];
  const [
    _,
  ...
  const ret = {
    ...
    parameters: split(parameters)
      .map(fixNameType)
      .filter(Boolean) as TextName[],
    typeParameters: typeParams,
    returnType: returnType?.trim(),
    returnTypeParameters: split(returnTypeParameters),
    imports: f
      .filter((v) => v.kind === KindEnum.TypeIdentifier)
      .filter((v) => !(typeParams.includes(v.text) || v.text == 'Self')),
  };
