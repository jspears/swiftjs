...
export class SwiftDocImpl {
    ...
    inherited() {
        const int: string[] = (this.doc.relationshipsSections?.find(({ type, kind }: RelationshipsSection) => (type === 'inheritsFrom' && kind == 'relationships'))?.identifiers)
            || [];
    ...
    get properties() {
        ...
        return Object.values(this.references).filter((v: ReferenceType) => {
            return v?.identifier.startsWith(this.identifier + '/') && v?.role === 'symbol';
        });
    }
    get methods() {
        return this.properties?.filter(({ fragments = [] }) => fragments?.[0]?.kind === KindEnum.Keyword && fragments?.[0]?.text === 'func') || [];
    }
    ...
    addProtocol(ref: ReferenceType) {
        ...
        inherited.forEach(({ url }) => this.generator.registerType(url.replace('`/documentation/swiftui', '')));
    ...
    addType(type: string) {
        ...
        if (!this.source.getClass(type) && !this.source.getImportDeclaration(`./${type}`)) {
    ...
    addPropertyTo(clz: ClassDeclaration, type: ReferenceType): this {
        const { returnType = 'this', returnTypeParameters = [], imports, ...method } = fragToMethod(type['fragments']);
        if (returnType && !returnTypeParameters.includes(returnType)) {
            this.addType(returnType);
        }

        method.parameters?.forEach(v => (v && !returnTypeParameters.includes(v.type) && this.addType(v?.type)));
        ...
        const m = clz.addMethod({
            ...
            returnType: writer => {
                ...
                if (returnTypeParameters.length) {
                    writer.write(`<${returnTypeParameters.map(v => v == 'Self' ? 'this' : v.trim()).join(',')}>`);
                }
            }
        });
