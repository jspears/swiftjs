  ...
  constructor(parameter: string, public addType: AddType, private count = 0) {
    ...
    this.params = splitParams.map((v) => {
      ...
      if (closure) {
        ...
        if (typeof type === 'string') hasQuestionToken = type.endsWith('?');
        if (hasQuestionToken) {
...
export class ClosureImpl {
  public name: string = '';
  static parseMethod(f: Fragment[] | string = [], addType: AddType) {
  ...
  constructor(
    ...
    if (!constraint.includes(_returnType)) params.addType(_returnType);
