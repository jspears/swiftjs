...
type KeyRet<A extends Constructor, K> = K extends undefined
  ? undefined
  : K extends string
  ? K extends `.${infer Key extends keyof A & string}`
    ? A[Key]
    : never
  : InstanceType<A>;

type FactoryKey<A extends Constructor> = ReturnType<typeof swifty<A>> & {
  fromKey<K extends KeyDot<A>>(k: K): KeyRet<A, K>;
};

export function swiftyKey<A extends Constructor>(clazz: A) {
